...
一次显示 9 个里程（当前轮 ±4），其中**当前轮突出、紧邻两轮稍凸显、其余为常规刻度**，
随阅读位置平滑滑动；两侧的 `⌃`/`⌄` 提示还有更多里程。

## 刻度强调与滑动动画

刻度按**距当前对话的远近分三档**（`emphasisOf`，见 locator.ts），像轮播指示器：

| 档位 | 是谁 | 表现 | 宽度 |
|---|---|---|---|
| 突出 | 当前对话那一轮 | 最长、最亮、最粗，带端点 | 24px |
| 稍凸显 | 当前轮的**左右紧邻**各一轮 | 比其余更长更亮 | 18px |
| 正常 | 其余所有轮次 | 清晰可见的常规刻度（**不会淡到看不见**）| 12px |

> 三档宽度按参考图比例标定（最长 ≈100%、邻档 ≈75%、其余 ≈45–50%）。此前的实现让外围刻度
> 淡出到 `opacity .12` + 强模糊，观感像"没渲染出来"；现已改为只降级不隐藏——**弱化不能弱到像缺失**。

切换是**一次连续的滑动**，不是刻度原地替换：

- 视口一次显示 9 个刻度（`VISIBLE_TICKS`，当前轮 ±4），另每侧多渲染 2 个过渡刻度
  （`STRIP_OVERSCAN`），共 `STRIP_TICKS = 13` 个。
- 整列用一个 `translateY` 平移，让焦点刻度落在视口中心线；CSS `transition: transform .34s` 负责补间。
- 多渲染的那 4 个刻度是**过渡材料**——没有它们，走一格就只能卸载/挂载节点，看起来是"顶替"。
- 视口边缘用极轻的 `mask-image` 软化最外侧一行，让它像"从边缘浮现"而非被硬切；这只做软化，
  不负责强调（强调全在 `data-emphasis` 三档里）。
...
4. 浏览器：标尺出现在**对话区左侧**（垂直排列）；一次显示 9 个刻度，其中**当前轮最长最亮**、
   其**紧邻两轮次之**、其余为**清晰可见的常规刻度**（不应有任何刻度淡到看不清）；
   滚动或点击时焦点刻度**平滑滑到中心**（不是原地替换）；hover 出卡；点击跳转；
   里程多于显示区时两端出现 `⌃`/`⌄`。
...
npm test           # 74 个纯函数单测（折叠状态机 + 可视轮判定 + 滑动 strip + 居中量 + 强调分档 + 相对时间）
